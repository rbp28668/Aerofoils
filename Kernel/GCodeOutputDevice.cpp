...
void GCodeOutputDevice::startPlot()
{
	...
	if (config->preCorrectGeometry) {
		oss << "(--PRE_CORRECT_GEOMETRY=TRUE)" << endl;
		oss << "(--CUTTER_WIDTH=" << geometry->getWidth() << ")" << endl;
		oss << "(--BLOCK_LEFT=" << geometry->getBlockLeft() << ")" << endl;
		oss << "(--BLOCK_RIGHT=" << geometry->getBlockRight() << ")" << endl;
	}
